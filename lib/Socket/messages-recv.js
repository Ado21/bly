...
const makeMessagesRecvSocket = (config) => {
    ...
    const lidToJidCache = config.lidToJidCache || new node_cache_1.default({
        stdTTL: 60 * 60 * 24,
        useClones: false
    });

    const groupMetadataCache = config.groupMetadataCache || new node_cache_1.default({
        stdTTL: 60 * 5,
        useClones: false
    });

    const pendingGroupMetadata = new Map();
    const getGroupMetadataCached = async (jid) => {
        const cached = groupMetadataCache.get(jid);
        if (cached) return cached;
        if (pendingGroupMetadata.has(jid)) return pendingGroupMetadata.get(jid);
        const promise = groupMetadata(jid).finally(() => setTimeout(() => pendingGroupMetadata.delete(jid), 1000));
        pendingGroupMetadata.set(jid, promise);
        const result = await promise;
        groupMetadataCache.set(jid, result);
        return result;
    };

    const resolveLidToJid = (lid, fallbackJid) => {
        if (!lid.includes('@lid')) return lid;
        const cached = lidToJidCache.get(lid);
        if (cached) return cached;
        const resolved = fallbackJid || lid.replace('@lid', '@s.whatsapp.net');
        lidToJidCache.set(lid, resolved);
        return resolved;
    };
        ...
        await authState.keys.transaction(async () => {
            const receipt = {
                ...
                content: [
                    ...
                    { tag: 'registration', attrs: {}, content: (0, Utils_1.encodeBigEndian)(authState.creds.registrationId) }
                ]
            };
    ...
    const handleGroupNotification = (participant, child, msg) => {
        ...
        switch (child?.tag) {
            ...
            case 'ephemeral':
            case 'not_ephemeral':
                msg.message = { protocolMessage: { type: WAProto_1.proto.Message.ProtocolMessage.Type.EPHEMERAL_SETTING, ephemeralExpiration: +(child.attrs.expiration || 0) } };
                break;
            case 'modify':
    ...
    const processNotification = async (node) => {
        ...
        switch (nodeType) {
                ...
                if (child.tag === 'disappearing_mode') {
                    ...
                    ev.emit('creds.update', {
                        accountSettings: {
                            ...
                            defaultDisappearingMode: { ephemeralExpiration: newDuration, ephemeralSettingTimestamp: timestamp },
                        }
                    });
            ...
            case 'link_code_companion_reg':
                ...
                const linkCodePairingExpanded = await (0, Utils_1.hkdf)(companionSharedKey, 32, { salt: linkCodeSalt, info: 'link_code_pairing_key_bundle_encryption_key' });
                ...
                await query({
                    tag: 'iq',
                    attrs: { to: WABinary_1.S_WHATSAPP_NET, type: 'set', id: sock.generateMessageTag(), xmlns: 'md' },
                    content: [
                        {
    ...
    function toRequiredBuffer(data) {
        if (data === undefined) throw new boom_1.Boom('Invalid buffer', { statusCode: 400 });
        ...
        try {
            await Promise.all([
                processingMutex.mutex(async () => {
                    ...
                    if (typeof status !== 'undefined' && (status >= WAProto_1.proto.WebMessageInfo.Status.SERVER_ACK || !isNodeFromMe)) {
                        if ((0, WABinary_1.isJidGroup)(remoteJid) || (0, WABinary_1.isJidStatusBroadcast)(remoteJid)) {
                            if (attrs.participant) {
                                const updateKey = status === WAProto_1.proto.WebMessageInfo.Status.DELIVERY_ACK ? 'receiptTimestamp' : 'readTimestamp';
                                ev.emit('message-receipt.update', ids.map(id => ({
                                    key: { ...key, id },
                                    receipt: { userJid: (0, WABinary_1.jidNormalizedUser)(attrs.participant), [updateKey]: +attrs.t }
                                })));
                            }
                        } else {
    ...
    const handleMessage = async (node) => {
        const { attrs } = node;
        if (shouldIgnoreJid(attrs.from) && attrs.from !== '@s.whatsapp.net') {
            logger.debug({ key: attrs }, 'ignored message');
            await sendMessageAck(node);
            return;
        }
        const encNode = (0, WABinary_1.getBinaryNodeChild)(node, 'enc');
        if (encNode && encNode.attrs.type === 'msmsg') {
            logger.debug({ key: attrs }, 'ignored msmsg');
            await sendMessageAck(node);
            return;
        }
        let response;
        if ((0, WABinary_1.getBinaryNodeChild)(node, 'unavailable') && !encNode) {
            await sendMessageAck(node);
            const { fullMessage } = (0, Utils_1.decodeMessageNode)(node, authState.creds.me.id, authState.creds.me.lid || '');
            response = await requestPlaceholderResend(fullMessage.key);
            if (response === 'RESOLVED') return;
            logger.debug('requested placeholder resend');
        } else {
            placeholderResendCache.del(attrs.id);
        }
        const { fullMessage: msg, category, author, decrypt } = (0, Utils_1.decryptMessageNode)(
            node, authState.creds.me.id, authState.creds.me.lid || '', signalRepository, logger
        );

        msg.key.remoteJid = resolveLidToJid(msg.key.remoteJid, attrs.sender_pn || attrs.peer_recipient_pn);
        msg.key.participant = msg.key.participant ? resolveLidToJid(msg.key.participant, attrs.participant_pn) : undefined;
        if (attrs.sender_pn && attrs.from.includes('@lid')) {
            lidToJidCache.set(attrs.from, attrs.sender_pn);
        }
                ...
                processingMutex.mutex(async () => {
                    ...
                    if (msg.messageStubType === WAProto_1.proto.WebMessageInfo.StubType.CIPHERTEXT) {
                        ...
                        retryMutex.mutex(async () => {
                            if (ws.isOpen && !(0, WABinary_1.getBinaryNodeChild)(node, 'unavailable')) {
                    ...
                    if ((0, WABinary_1.isJidGroup)(msg.key.remoteJid) && contextInfo?.participant?.includes('@lid')) {
                        try {
                            const metadata = await getGroupMetadataCached(msg.key.remoteJid);
                            const lidParticipant = contextInfo.participant;
                            const found = metadata.participants.find(p => p.lid === lidParticipant || p.id === lidParticipant);
                            if (found?.id) {
                                contextInfo.participant = found.id;
                                lidToJidCache.set(lidParticipant, found.id);
                            }
                        } catch (err) {
                            logger.warn({ group: msg.key.remoteJid, err }, 'failed to resolve contextInfo participant');
                        }
                    }

                    if (msg.key.participant?.includes('@lid')) {
                        msg.key.participant = resolveLidToJid(msg.key.participant, attrs.participant_pn);
                    }
                    ...
                    if (category === 'peer') {
                        ...
                        if ((0, WABinary_1.isJidUser)(msg.key.remoteJid)) {
                            msg.key.participant = author;
                        }
                    } else if (!sendActiveReceipts) {
                        type = 'inactive';
                    }

                    await sendReceipt(msg.key.remoteJid, msg.key.participant, [msg.key.id], type);
                    if ((0, Utils_1.getHistoryMsg)(msg.message)) {
                        await sendReceipt((0, WABinary_1.jidNormalizedUser)(msg.key.remoteJid), undefined, [msg.key.id], 'hist_sync');
                    }
    ...
    const fetchMessageHistory = async (count, oldestMsgKey, oldestMsgTimestamp) => {
        if (!authState.creds.me?.id) throw new boom_1.Boom('Not authenticated');
        const pdoMessage = {
    ...
    const requestPlaceholderResend = async (messageKey) => {
        ...
        setTimeout(() => {
            if (placeholderResendCache.get(messageKey?.id)) {
                logger.debug({ messageKey }, 'PDO message without response after 15 seconds');
    ...
    const handleBadAck = async ({ attrs }) => {
        ...
        if (attrs.phash) {
            ...
            if ((msgRetryCache.get(cacheKey) || 0) >= maxMsgRetryCount) {
                logger.warn({ attrs }, 'reached max retry count');
            ...
            if (msg) {
                ...
                logger.warn({ attrs }, 'could not send message again');
            }
        }
        if (attrs.error) {
            ...
            ev.emit('messages.update', [{
                key,
                update: { status: Types_1.WAMessageStatus.ERROR, messageStubParameters: [attrs.error] }
            }]);
        }
    };
    ...
    ws.on('CB:call', async (node) => processNode('call', node, 'handling call', handleCall));
    ws.on('CB:receipt', node => processNode('receipt', node, 'handling receipt', handleReceipt));
    ws.on('CB:notification', async (node) => processNode('notification', node, 'handling notification', handleNotification));
